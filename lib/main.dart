...
Page resource error:
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('SPOS sandbox-1 test'),
...
class SampleMenu extends StatelessWidget {
    ...
    return PopupMenuButton<MenuOptions>(
      ...
      onSelected: (MenuOptions value) {
        switch (value) {
          ...
          case MenuOptions.checkConnectivity:
            _checkConnectionAndNotifyWebView();
          case MenuOptions.updateOffline:
            _updateOffline();
          case MenuOptions.updateOnline:
            _updateOnline();
        }
      },
      itemBuilder: (BuildContext context) => <PopupMenuItem<MenuOptions>>[
        ...
        const PopupMenuItem<MenuOptions>(
          value: MenuOptions.checkConnectivity,
          child: Text('Update connectivity'),
        ),
        const PopupMenuItem<MenuOptions>(
          value: MenuOptions.updateOffline,
          child: Text('Update connectivity (offline)'),
        ),
        const PopupMenuItem<MenuOptions>(
          value: MenuOptions.updateOnline,
          child: Text('Update connectivity (online)'),
        ),
      ],
    );
  }
  ...
  Future<void> _checkConnectionAndNotifyWebView() async {
    var connectivityResult = await Connectivity().checkConnectivity();
    bool isOnline = connectivityResult.contains(ConnectivityResult.wifi) ||
        connectivityResult.contains(ConnectivityResult.mobile);

    // Inject JavaScript to notify the WebView
    return webViewController.runJavaScript(
        "window.connectivity = ${isOnline ? 'online' : 'offline'};");
  }

  Future<void> _updateOffline() {
    return webViewController.runJavaScript("window.connectivity = 'offline'");
  }

  Future<void> _updateOnline() {
    return webViewController.runJavaScript("window.connectivity = 'online'");
  }
